...
@interface UIView (hookblock)

@end

@implementation UIView (hookblock)

+ (void)load{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        ExchangeImplementations([self class], @selector(qmui_layoutSubviewsBlock), @selector(hb_layoutSubviewsBlock));
    });
}

- (void)hb_layoutSubviewsBlock{
    
    NSLog(@"dddd%@",self);
    
    [self hb_layoutSubviewsBlock];
}
...
//    view.qmui_shouldShowUpdatesIndicator = YES;
    view.qmui_badgeInteger = 5;
    view.backgroundColor = UIColor.orangeColor;
    [self.view addSubview:view];
//    view.qmui_layoutSubviewsBlock(UIView.new);
//    view = nil;
//    view.qmui_layoutSubviewsBlock(UIView.new);// setQmui_layoutSubviewsBlock
}
