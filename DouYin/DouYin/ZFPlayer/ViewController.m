...
//    NSString *url = @"http://service.matafy.com/community/dynamic/recommend/list?dynamicType=VIDEO&page=1&size=30";
    ...
    [NetworkRequest postWithUrl:url para:para dataHandle:^(NSArray <DynamicListModelDataList *> *data) {
        ...
        for (DynamicListModelDataList *list in data) {
            ZFTableData *tableData = [[ZFTableData alloc] init];
            tableData.nick_name = list.fromCn;
            tableData.title = list.mediaContentList.lastObject.url;
            [self.dataSource addObject:tableData];
            [self.urls addObject:[NSURL URLWithString:list.mediaContentList.lastObject.url]];
        }
        self.player.assetURLs = self.urls;
        [self.tableView reloadData];
