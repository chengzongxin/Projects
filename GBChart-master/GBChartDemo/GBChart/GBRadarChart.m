...
//    rgba(205, 207, 222, 1)
    ...
    for (int i = 0; i < _pointsToWebArrayArray.count; i++) {
        ...
        // 最后一次绘制
        if (i == _pointsToWebArrayArray.count - 1) {
            CGContextSetLineWidth(ctx, 1);
            CGFloat lengths0[] = {0,0};
            CGContextSetLineDash(ctx, 0, lengths0, 0);
            CGContextSetStrokeColorWithColor(ctx, _webColor.CGColor);
        }
...
//    [path closePath];
    ...
    //绘制渐变色
    [self drawLinearGradientWithContext:ctx path:path beginColor:[UIColor colorWithRed:127/255.0 green:236/255.0 blue:233/255.0 alpha:1.0].CGColor endColor:[UIColor colorWithRed:138/255.0 green:220/255.0 blue:242/255.0 alpha:1.0].CGColor];
}
