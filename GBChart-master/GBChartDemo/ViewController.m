...
- (void)radarChart {
    ...
    for (int i = 0; i < values.count; i++) {
//        NSAttributedString *string = [[NSAttributedString alloc] initWithString:text[i]];
//        GBRadarChartDataItem *item = [GBRadarChartDataItem dataItemWithValue:[values[i] floatValue] description:descs[i]];
    ...
    GBRadarChart *radarChart = [[GBRadarChart alloc] initWithFrame:CGRectMake(margin, 100, width, width) items:items valueDivider:20];
